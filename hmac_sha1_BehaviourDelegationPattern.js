...
/* var yourHmacSha1 =  */ (function(){ 
  ...
  var hmacSha1 = {};
  hmacSha1.setConstants = function(){  // set constants by puttng them in private object called pocket
     ...
     (function applyXor(){   // Reads one char, at the time, from key and applies XOR constants on it
        ...
        for(var j = 0; j < this.blocksize; j++){ 
           ...
           if(diff && (j+diff) >= this.blocksize || j >= hashedKeyLen){ // if diff exists (key is shorter then
               ...
               o_paddedCode =  charCode ^ pocket.opad; // XOR the char code with outer padding constant (opad)
               opad_key += String.fromCharCode(o_paddedCode); // convert back code result to string
                  
               i_paddedCode = charCode ^ pocket.ipad;  // XOR with the inner padding constant (ipad)
  ...
  hmacSha1.oneByteCharAt = function (str,idx){
  ...
  hmacSha1.hexToString = function (sha1Output){ // Converts every pair of hex CHARS to their character
                                                // conterparts.
 ...
 function AttachPocket (){ // Fucntion that attaches private object (pocket) to new "instance" of hmacSha1 and 
     ...
     var publicAPI = Object.create(hmacSha1); //  making new "instance" of hmacSha1 , linking prototype chains.
     publicAPI.digest = function(){
         
         var args = Array.prototype.slice.call(arguments);
         args = args.concat([pocket]); // add pocket as last argument
         return hmacSha1.digest.apply(this, args)
            
     }
     publicAPI.setConstants = function(){
         var args = Array.prototype.slice.call(arguments);
         args = args.concat([pocket]); // add pocket as last argument
         
         return hmacSha1.setConstants.apply(this, args)
     }    
